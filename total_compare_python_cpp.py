...
def load_and_compare(cpp_file_path, py_file_path, base_seq):
    ...
    try:
        ...
        # 计算余弦相似度 tensor
        sim = cosine_similarity(hidden_cpp, hidden_py, dim=-1)
...
def main():
    # 目录路径
    cpp_dir = "/export/home/weinan5/wangshuibin/10_new_flux2_tp_xllm/dump_flux2_tensor/08_cpp_single_inner_parallelattention_tensor/total_attention_tensor"
    py_dir = "/export/home/weinan5/wangshuibin/10_new_flux2_tp_xllm/02_python_tensor/08_python_single_inner_parallel_attention_tensor"
    ...
    for base_seq in sorted_seqs:
        ...
        if sim is not None:
            results.append({
                ...
                'C++文件': cpp_files[0],
                'Python文件': py_files[0],
                '余弦相似度均值': sim.mean().item(),
                '最大绝对误差': max_abs_diff,
                '张量形状': str(shape),
                '数据类型': str(dtype)
            })
            
            print(f"  余弦相似度tensor: {sim}")
            print(f"  余弦相似度tensor形状: {sim.shape}")
            print(f"  最大绝对误差: {max_abs_diff:.8f}")
            print(f"  张量形状: {shape}")
            print(f"  数据类型: {dtype}")
        else:

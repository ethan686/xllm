...
class MistralRotaryEmbeddingImpl : public torch::nn::Module {
 public:
   ...
   * @param rope_theta Base for the rotational frequency
   */
  ...
  std::pair<torch::Tensor, torch::Tensor> forward(
    ...
    auto inv_freq_expanded = inv_freq_.expand({batch_size, -1, -1})
                                         .to(torch::kFloat32);
...
class MistralDecoderLayerImpl : public LlmDecoderLayerImplBase<layer::NpuMistralDecoderLayer> {
 public:
  MistralDecoderLayerImpl(const ModelContext& context, int32_t layer_id)
      : LlmDecoderLayerImplBase<layer::NpuMistralDecoderLayer>(context, layer_id) {
    try {
      decoder_layer_ = std::make_shared<layer::NpuMistralDecoderLayer>(context);
      register_module("decoder_layer", decoder_layer_);
    } catch (const std::exception& e) {
      LOG(ERROR) << "Failed to create MistralDecoderLayer: " << e.what();
      throw;
    }
  }
  ...
  torch::Tensor forward(
      torch::Tensor& x,
      const torch::Tensor& m_cos_pos,
      const torch::Tensor& m_sin_pos,
      const torch::Tensor& cu_seq_len,
      const std::vector<int>& cu_seq_len_vec,
 ...
 private:
  void check_initialized() const {
    TORCH_CHECK(decoder_layer_ != nullptr, 
                "Decoder layer not initialized. Call constructor first.");
  }

  std::shared_ptr<layer::NpuMistralDecoderLayer> decoder_layer_{nullptr};
};
...
class MistralModelImpl : public LlmModelImplBase<MistralDecoderLayer> {
 public:
  ...
  explicit MistralModelImpl(const ModelContext& context)
    ...
    create_embedding(model_args, options);
    
    // 2. Decoder layers
    create_decoder_layers(context, model_args);
    
    // 3. Final layer normalization
    create_final_norm(model_args);
    
    // 4. Rotary position embeddings
    create_rotary_embedding(model_args);
    
    // 5. Attention mask layer (lazy initialization)
    attention_mask_layer_ = std::make_shared<layer::AttentionMask>();
    
    LOG(INFO) << "MistralModel initialized with " << model_args.mm_num_hidden_layers() 
              << " layers, hidden size " << hidden_size_;
  }
  ...
  Output forward(
    ...
    // 1. Get hidden states
    torch::Tensor hidden_states = get_hidden_states(input_ids, inputs_embeds);
    
    const auto batch_size = hidden_states.size(0);
    const auto seq_len = hidden_states.size(1);
    
    // 2. Prepare position IDs
    torch::Tensor pos_ids = get_position_ids(position_ids, past_key_values, 
                                             use_cache, seq_len, batch_size, 
                                             hidden_states.device());
    
    // 3. Get rotary position embeddings
    auto [cos, sin] = rotary_emb_->forward(hidden_states, pos_ids);
    
    // 4. Prepare cu_seq_len for variable sequences
    auto [cu_seq_len, cu_seq_len_vec] = prepare_seq_len_info(
        attention_mask, batch_size, seq_len, hidden_states.device());
    
    // 5. Prepare input parameters
    ModelInputParams input_params = prepare_input_params(
        batch_size, cu_seq_len_vec, past_key_values, use_cache);
    
    // 6. Process through decoder layers
    auto [final_hidden_states, all_hidden_states] = process_layers(
        hidden_states, cos, sin, cu_seq_len, cu_seq_len_vec, 
        input_params, output_hidden_states);
    
    // 7. Final normalization
    final_hidden_states = norm_->forward(final_hidden_states);
  ...
  void load_state_dict(const StateDict& state_dict) {
    ...
    LOG(INFO) << "MistralModel state dict loaded successfully";
  }
  ...
  void verify_loaded_weights(const std::string& prefix) const {
    ...
    LOG(INFO) << "MistralModel weights verified successfully";
  }
 ...
 private:
  ...
  void create_embedding(const ModelArgs& model_args, 
                        const std::optional<torch::TensorOptions>& options) {
  ...
  void create_decoder_layers(const ModelContext& context, 
  ...
  void create_final_norm(const ModelArgs& model_args) {
  ...
  void create_rotary_embedding(const ModelArgs& model_args) {
  ...
  torch::Tensor get_hidden_states(const torch::Tensor& input_ids,
                                   const torch::Tensor& inputs_embeds) const {
  ...
  torch::Tensor get_position_ids(const torch::Tensor& position_ids,
                                  const std::shared_ptr<Cache>& past_key_values,
                                  bool use_cache,
                                  int64_t seq_len,
                                  int64_t batch_size,
    ...
    return torch::arange(seq_len, device)
               .unsqueeze(0)
               .expand({batch_size, -1})
               .add(past_len);
  }

  std::pair<torch::Tensor, std::vector<int>> prepare_seq_len_info(
      const torch::Tensor& attention_mask,
      int64_t batch_size,
      int64_t seq_len,
      c10::Device device) const {
    ...
    if (attention_mask.defined()) {
      auto seq_lens = attention_mask.sum(-1);
    ...
    auto cu_seq_len = torch::tensor(cu_seq_len_vec, torch::kInt32).to(device);
    return {cu_seq_len, cu_seq_len_vec};
  }

  ModelInputParams prepare_input_params(int64_t batch_size,
                                         const std::vector<int>& cu_seq_len_vec,
                                         const std::shared_ptr<Cache>& past_key_values,
  ...
  std::pair<torch::Tensor, std::vector<torch::Tensor>> process_layers(
    ...
    for (size_t i = 0; i < layers_.size(); ++i) {
      hidden_states = layers_[i]->forward(
          hidden_states, cos, sin, cu_seq_len, cu_seq_len_vec, 
          input_params, static_cast<int>(i)
      );
    ...
    return {hidden_states, std::move(all_hidden_states)};
  }
...
class MistralCache : public Cache {
 ...
 private:
  void ensure_cache_size(size_t required_size) {
    if (key_cache_.size() < required_size) {
      key_cache_.resize(required_size);
      value_cache_.resize(required_size);
    }
  }

  void validate_layer_idx(int64_t layer_idx) const {

 ...
 private:
  ...
  torch::Tensor forward_impl(
    ...
    /*
        ...
        int64_t max_area = height * width;  // size param treated as area
    constraint
        ...
        int64_t ow1 = static_cast<int64_t>(std::sqrt(max_area * ratio)) / dw *
    dw; int64_t oh1 = (max_area / ow1) / dh * dh; double ratio1 =
    static_cast<double>(ow1) / oh1;

        // Candidate 2: floor height first
        int64_t oh2 = static_cast<int64_t>(std::sqrt(max_area / ratio)) / dh *
    dh; int64_t ow2 = (max_area / oh2) / dw * dw; double ratio2 =
    static_cast<double>(ow2) / oh2;
    ...
    << dh << ")";
        ...
        if (width % dw != 0) {
            width = (width / dw) * dw;
            LOG(WARNING) << "Width adjusted to " << width << " (multiple of " <<
    dw << ")";
        }
    } else {

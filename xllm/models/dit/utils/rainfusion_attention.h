...
struct RainFusionConfig {
  ...
  bool enabled = false;
  std::string version = "v2";      // "v2" or "v3"
  int64_t inner_precise = 0;       // V3: 0=float32 softmax, 1=fp16
  int64_t mask_refresh_steps = 1;  // V3: recompute mask every N steps
};

// Per-request dynamic state for RainFusion attention.
struct RainFusionState {
  int64_t current_step = 0;
  std::vector<int64_t> latent_shape = {1, 1, 1};
  int64_t seq_len = -1;
  torch::Tensor cached_mask;  // V3: cached binary int8 block_sparse_mask
};
...
// =========================================================================
//  V3 block-decompose sparse attention (aclnnBlockSparseAttention)
//  Ported from wan22_rainfusion branch.
//  Operates directly on BNSD [B,N,S,D] — no to_flat/from_flat.
// =========================================================================

// V3 rearrange: block-decompose (f hn hb wn wb) → (f hn wn hb wb).
...
// V3 inverse rearrange — reverses rearrange_v3.
...
// V3 binary mask for aclnnBlockSparseAttention.  Returns int8
...
// RainFusionV3 — main entry point.  Ported from wan22_rainfusion.
inline std::tuple<torch::Tensor, torch::Tensor> rainfusion_attention_v3(
    const torch::Tensor& query,
    const torch::Tensor& key,
    const torch::Tensor& value,
    const RainFusionConfig& config,
